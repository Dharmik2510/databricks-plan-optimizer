    ...
    const runAnalysis = async () => {
      for (let i = 0; i < stages.length; i++) {
        ...
        await new Promise(resolve => {
          stageTimer = setTimeout(resolve, stageDuration);
        });
      }
    ...
    return (
      <div className="flex items-start gap-4 mb-6 last:mb-0 relative">
        ...
        {index < stages.length - 1 && (
          <div className={`absolute left-7 top-14 bottom-0 w-0.5 h-8 -mb-4 transition-colors duration-500 ${isCompleted ? 'bg-slate-300' : 'bg-slate-100'}`}></div>
        )}
        ...
        <div className="flex-shrink-0 relative z-10">
          <div className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-all duration-500 ${
            isCompleted 
              ...
              : isActive 
              ? `bg-gradient-to-br ${color} text-white shadow-lg shadow-orange-500/20 scale-110 ring-4 ring-white` 
        ...
        <div className="flex-1 pt-1.5">
          <div className="flex justify-between items-center mb-1">
            ...
            {isActive && <span className="text-xs font-bold text-slate-400">{Math.round(progress)}%</span>}
          </div>
          <p className={`text-xs transition-colors duration-300 mb-2 ${isActive ? 'text-slate-600' : 'text-slate-300'}`}>
            {description}
          </p>
          
          {/* Progress Bar */}
          <div className={`h-1.5 w-full rounded-full overflow-hidden bg-slate-100 transition-all duration-500 ${isActive ? 'opacity-100' : 'opacity-0'}`}>
            <div 
              className={`h-full rounded-full transition-all duration-100 ease-linear bg-gradient-to-r ${color}`}
      ...
      <div className="w-full max-w-2xl">
        ...
        <div className="bg-white rounded-3xl shadow-xl border border-slate-200 p-8 md:p-10 relative overflow-hidden">
          ...
          <div className="text-center mb-10 relative z-10">
            <div className="inline-flex items-center gap-2 mb-2 px-3 py-1 rounded-full bg-slate-50 border border-slate-100 text-xs font-bold text-slate-500 uppercase tracking-wider">
              <Sparkles className="w-3 h-3 text-orange-500" /> AI Optimization in Progress
            </div>
            <h2 className="text-3xl font-bold text-slate-900 mb-2">Analyzing Execution Plan</h2>
            <p className="text-slate-500 font-medium">Please wait while we deconstruct your Spark DAG.</p>
          </div>
          ...
          <div className="mt-10 pt-6 border-t border-slate-100 relative z-10">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <span className="text-xs font-bold text-yellow-600 uppercase tracking-wide block mb-1">Optimization Tip</span>
                <p className="text-sm text-slate-600 font-medium leading-relaxed animate-fade-in" key={tipIndex}>
                  {TIPS[tipIndex]}
                </p>
              </div>
            </div>
        ...
        <div className="text-center mt-6 text-slate-400 text-xs font-medium">
          Powered by Gemini 2.5 Flash • Secure Analysis Environment
        </div>
