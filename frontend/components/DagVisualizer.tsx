...
export const DagVisualizer: React.FC<DagVisualizerProps> = ({ nodes, links }) => {
  ...
  const renderGraph = (width: number) => {
    ...
    const svg = d3.select(svgRef.current)
      ...
      .style("cursor", "grab")
      .on("touchstart", (event: any) => event.preventDefault()); // Prevent scrolling while touching canvas
    ...
    const initialTransform = d3.zoomIdentity
      .translate(width < 640 ? 20 : 50, height / 2) // Less padding on mobile
      .scale(width < 640 ? 0.6 : 0.8); // Smaller scale on mobile
    ...
    const simNodes: SimulationNode[] = nodes.map(n => ({
      ...n,
      x: (levels.get(n.id) || 0) * (width < 640 ? 150 : 200), // Tighter spacing on mobile
    ...
    const simulation = d3.forceSimulation(simNodes)
      .force("link", d3.forceLink(simLinks).id((d: any) => d.id).distance(width < 640 ? 140 : 180))
      .force("charge", d3.forceManyBody().strength(width < 640 ? -1000 : -1500))
      .force("collide", d3.forceCollide().radius(80))
      .force("x", d3.forceX((d: any) => (levels.get(d.id) || 0) * (width < 640 ? 160 : 220)).strength(1.5))
    ...
    // Node Text
    const getWrapText = (selection: any) => {
  ...
  useEffect(() => {
    ...
    // Initial render
    renderGraph(containerRef.current.clientWidth);

    const resizeObserver = new ResizeObserver(entries => {
      for (let entry of entries) {
        if (entry.contentRect.width > 0) {
          renderGraph(entry.contentRect.width);
        }
      }
    });
  ...
  return (
    <div ref={containerRef} className="w-full bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden flex flex-col h-[400px] sm:h-[500px] lg:h-[600px] relative group transition-all duration-300">
      <div className="p-5 border-b border-slate-200 bg-slate-50 flex justify-between items-center flex-shrink-0">
        ...
        <div className="flex items-center gap-3">
          <div className="flex gap-3 mr-4 border-r border-slate-300 pr-4 hidden sm:flex">
            <div className="flex items-center gap-1.5 text-[10px] text-slate-700 font-bold"><span className="w-2 h-2 rounded-full bg-green-500 shadow-sm"></span> Scan</div>
            <div className="flex items-center gap-1.5 text-[10px] text-slate-700 font-bold"><span className="w-2 h-2 rounded-full bg-red-500 shadow-sm"></span> Shuffle</div>
            <div className="flex items-center gap-1.5 text-[10px] text-slate-700 font-bold"><span className="w-2 h-2 rounded-full bg-cyan-500 shadow-sm"></span> Transform</div>
          </div>
      ...
      <div className="flex-1 relative overflow-hidden bg-white">
        ...
        <svg ref={svgRef} className="w-full h-full block relative z-10 touch-none"></svg>
      </div>
    </div>
  );
};