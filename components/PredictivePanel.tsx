  ...
  return (
    <div className="space-y-8 animate-fade-in mt-12">
      <div className="flex items-center gap-4 mb-4">
        ...
        <div>
           <h3 className="text-2xl font-bold text-slate-900 drop-shadow-sm">Predictive Analytics & Scalability</h3>
           <p className="text-slate-600 font-medium">AI-driven projection of workload performance at scale.</p>
        </div>
      </div>
        ...
        <div className="grid grid-cols-2 gap-6">
            ...
            <div className="bg-emerald-50/40 backdrop-blur-3xl rounded-3xl border border-emerald-100/50 p-6 shadow-lg ring-1 ring-emerald-100/30 flex flex-col justify-center">
                ...
                <div className="text-3xl font-bold text-emerald-700">
                    {(prediction.predictedExecutionTime / 60).toFixed(1)} <span className="text-base text-emerald-600 font-medium">min</span>
                </div>
                ...
                {prediction.bottleneckProgression.map((b, i) => (
                    <div key={i} className="bg-white/50 rounded-xl p-4 border border-white/60">
                        ...
                        <div className="flex items-center gap-4 text-xs font-medium text-slate-600 mb-2">
                             <div className="flex-1">Current Impact: <span className="text-slate-900">{b.currentImpact}%</span></div>
                             <div className="flex-1">At 10x: <span className="text-amber-700 font-bold">{b.at10xScale}%</span></div>
                             <div className="flex-1">At 100x: <span className="text-red-700 font-bold">{b.at100xScale}%</span></div>
                        </div>
                        <div className="text-xs text-slate-700 italic border-t border-slate-200 pt-2 mt-2">
                            "{b.recommendation}"
                        </div>
                    </div>
                ))}
                 ...
                 {prediction.whatIfScenarios.map((s, i) => (
                     <div key={i} className="bg-white/70 rounded-xl p-4 border border-white/60 shadow-sm transition-transform hover:scale-[1.02]">
                         ...
                         <div className="grid grid-cols-2 gap-2 text-xs">
                             <div className="text-slate-500">Reduction: <span className="text-emerald-600 font-bold">{s.timeReduction}</span></div>
                             <div className="text-slate-500">Savings: <span className="text-emerald-600 font-bold">{s.costSavings}</span></div>
                             <div className="text-slate-500">Effort: <span className="text-slate-700 font-bold">{s.complexity}</span></div>
                             <div className="text-slate-500">Time: <span className="text-slate-700 font-bold">{s.implementation}</span></div>
                         </div>
                     </div>
                 ))}
             </div>
