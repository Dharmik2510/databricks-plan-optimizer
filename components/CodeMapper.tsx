          ...
          <div key={idx} className="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden group hover:border-blue-300 transition-all relative">
            ...
            <div className="flex items-center justify-between px-6 py-4 bg-slate-50 border-b border-slate-200">
              <div className="flex items-center gap-3">
                ...
                <span className={`px-2 py-1 rounded-full text-[10px] font-bold uppercase ${
                  map.matchType === 'exact' ? 'bg-green-100 text-green-700 border border-green-200' :
                  map.matchType === 'partial' ? 'bg-amber-100 text-amber-700 border border-amber-200' :
            ...
            <div className="p-6">
                  ...
                  <div className="flex items-center gap-2 text-xs font-mono text-blue-800">
                    {map.callStack.map((func, i) => (
                      <React.Fragment key={i}>
                        <span className="bg-white px-2 py-1 rounded border border-blue-200">{func}</span>
              ...
              <div className="relative group/code">
                <pre className="relative text-sm font-mono text-blue-100 bg-slate-900 p-4 rounded-2xl border border-slate-800 overflow-x-auto shadow-inner">
                  <code>{map.code}</code>
                </pre>
              </div>
            </div>
