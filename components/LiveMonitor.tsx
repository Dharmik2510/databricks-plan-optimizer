...
export const LiveMonitor: React.FC = () => {
  ...
  const handleConnect = async () => {
    ...
    // Attempt validation
    // In this demo environment, it will likely fail CORS or network, 
    // so we will simulate a successful connection after a short delay 
    // to show the UI capabilities.
    
    setTimeout(() => {
      setIsConnecting(false);
      setShowConfig(false);
      startSimulation(); // Fallback to simulation for visual demo
    }, 1500);
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in h-full flex flex-col relative">
      ...
      {showConfig && (
        <div className="absolute inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4 rounded-3xl">
          <div className="bg-slate-900 border border-white/10 rounded-3xl w-full max-w-md p-6 shadow-2xl relative overflow-hidden">
             ...
             <h3 className="text-xl font-bold text-white mb-6 flex items-center gap-2">
               <Settings className="w-5 h-5 text-indigo-400" /> Connect Cluster
             </h3>
             ...
             <div className="space-y-4">
               <div>
                 ...
                 <input 
                    ...
                    className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-sm text-white focus:border-indigo-500/50 outline-none"
                    value={config.host}
               ...
               <div>
                 ...
                 <div className="relative">
                   <input 
                      type="password" 
                      placeholder="dapi..." 
                      className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-sm text-white focus:border-indigo-500/50 outline-none pl-10"
                      value={config.token}
               ...
               <button 
                  ...
                  className="w-full bg-indigo-600 hover:bg-indigo-500 text-white font-bold py-3 rounded-xl mt-4 transition-all shadow-lg shadow-indigo-900/20 flex justify-center items-center gap-2"
               >
                 {isConnecting ? 'Connecting...' : 'Establish Connection'}
               </button>
               <p className="text-[10px] text-slate-500 text-center mt-2">
                 Note: Direct browser connection may be blocked by CORS. Ensure your workspace allows it or use a proxy.
               </p>
             </div>
          </div>
        </div>
      ...
      {/* Header Controls */}
      <div className="bg-slate-900/40 backdrop-blur-2xl rounded-3xl border border-white/10 p-6 flex justify-between items-center shadow-lg">
        ...
        <div className="flex gap-3">
          ...
          {status === StreamStatus.DISCONNECTED ? (
            <button onClick={() => setShowConfig(true)} className="flex items-center gap-2 px-6 py-3 bg-indigo-500/20 hover:bg-indigo-500/30 border border-indigo-500/50 text-indigo-300 rounded-xl font-bold transition-all shadow-[0_0_20px_rgba(99,102,241,0.1)] backdrop-blur-md">
      ...
      {status === StreamStatus.DISCONNECTED ? (
        <div className="flex-1 flex items-center justify-center bg-slate-900/30 backdrop-blur-xl rounded-3xl border border-white/10 border-dashed">
          ...
          {activeView === 'metrics' ? (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-full">
              ...
              <div className="lg:col-span-2 space-y-6 flex flex-col">
                ...
                {/* Throughput */}
                <div className="flex-1 bg-slate-900/40 backdrop-blur-2xl rounded-3xl border border-white/10 p-6 relative overflow-hidden group">
                  <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-white/20 to-transparent"></div>
                  <div className="flex justify-between items-center mb-4">
                     <h3 className="text-white font-bold flex items-center gap-2"><Database className="w-4 h-4 text-indigo-400"/> Throughput</h3>
                  </div>
                  <div className="h-56 w-full">
                    <ResponsiveContainer width="100%" height="100%">
                      <AreaChart data={metrics}>
                        ...
                        <Area type="monotone" dataKey="inputRate" stroke="#818cf8" strokeWidth={2} fill="url(#colorInput)" />
                ...
                {/* System Resources (CPU/Swap/Mem) */}
                <div className="flex-1 bg-slate-900/40 backdrop-blur-2xl rounded-3xl border border-white/10 p-6 relative overflow-hidden">
                  <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-white/20 to-transparent"></div>
                  <div className="flex justify-between items-center mb-4">
              ...
              {/* RIGHT COL: Logs */}
              <div className="bg-black/40 backdrop-blur-2xl rounded-3xl border border-white/10 overflow-hidden flex flex-col relative">
            ...
            // LIVE DAG VIEW
            <div className="h-full bg-slate-900/40 backdrop-blur-2xl rounded-3xl border border-white/10 p-6 relative overflow-hidden flex flex-col items-center justify-center">
                <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-white/20 to-transparent"></div>
                
                <div className="text-center space-y-4">
                    <div className="inline-block p-4 rounded-full bg-indigo-500/20 border border-indigo-500/30 shadow-[0_0_30px_rgba(99,102,241,0.2)]">
                ...
                {/* Mock Microbatch DAG visualization for demo */}
                <div className="mt-10 w-full max-w-3xl h-64 border border-white/10 rounded-2xl bg-black/20 relative flex items-center justify-around">
                    {/* Simple Flow for Demo */}
                ...
                {status === StreamStatus.CRITICAL && (
                    <div className="mt-6 flex items-center gap-2 text-red-400 text-sm bg-red-500/10 px-4 py-2 rounded-full border border-red-500/20 animate-pulse">
                        <Cpu className="w-4 h-4" /> Bottleneck Detected in Write Stage
                    </div>
                )}
            </div>
          )}
