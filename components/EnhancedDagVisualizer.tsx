...
export const EnhancedDagVisualizer: React.FC<Props> = ({ nodes, links, optimizations }) => {
  ...
  const estimateNodeCost = (node: DagNode): number => {
    // Rough cost heuristic based on operation type
  ...
  const enhancedLayout = useMemo(() => {
    ...
    nodes.forEach(n => {
      ...
      // Enrich node with bottleneck analysis
    ...
    // BFS layering with smarter positioning
    const levels = new Map<string, number>();
    const queue: string[] = [];
    
    // Find sources (Stage 0)
    nodes.forEach(n => {
  ...
  useEffect(() => {
    ...
    // Critical path gradient (red pulse)
