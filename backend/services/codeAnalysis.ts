...
import { RepoFile, DagNode, EnhancedCodeSnippet, CodeContext, FunctionDefinition, ImportStatement, TableReference, SparkOperation, DependencyInfo } from "../../shared/types";
...
export class CodeAnalysisEngine {
  ...
  private analyzeFile(file: RepoFile): CodeContext {
    ...
    switch (extension) {
      case 'py': return this.analyzePython(file);
      case 'scala': return this.analyzeScala(file);
      case 'sql': return this.analyzeSQL(file);
      case 'ipynb': return this.analyzeNotebook(file);
      default: return this.createEmptyContext(file);
    }
  }
  ...
  private analyzePython(file: RepoFile): CodeContext {
    ...
    const context: CodeContext = {
      file, functions: [], imports: [], tableReferences: [], sparkOperations: []
    };
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (sparkOp) {
        ...
        if (currentFunction) currentFunction.calls.push(sparkOp.type);
      }
  ...
  private analyzeScala(file: RepoFile): CodeContext {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.startsWith('import ')) {
        context.imports.push({ module: line.replace('import ', '').replace(';', '').trim(), items: [], line: i + 1 });
      }
      ...
      if (line.includes('from ') || line.includes('join ')) {
        ...
        tables.forEach(table => {
          context.tableReferences.push({
            name: table, operation: line.includes('join') ? 'join' : 'read', line: i + 1, context: lines[i]
          });
        });
      }
    }
  ...
  private analyzeNotebook(file: RepoFile): CodeContext {
    try {
      ...
      const codeContent = notebook.cells?.filter((cell: any) => cell.cell_type === 'code')?.map((cell: any) => cell.source.join('\n'))?.join('\n\n') || '';
      return this.analyzePython({ ...file, content: codeContent });
    } catch (error) { return this.createEmptyContext(file); }
  }

  private buildGlobalMaps() {
    for (const [filePath, context] of this.contexts) {
      context.functions.forEach(func => {
        this.globalFunctionMap.set(func.name, { file: filePath, line: func.startLine });
      });
      context.tableReferences.forEach(ref => {
        const files = this.tableToFileMap.get(ref.name) || [];
        if (!files.includes(filePath)) { files.push(filePath); this.tableToFileMap.set(ref.name, files); }
      });
    }
  }
  ...
  public mapDagNodesToCode(dagNodes: DagNode[]): EnhancedCodeSnippet[] {
    ...
    for (const node of dagNodes) {
      mappings.push(...this.mapSingleNode(node));
    }
  ...
  private mapSingleNode(node: DagNode): EnhancedCodeSnippet[] {
    const mappings: EnhancedCodeSnippet[] = [];
    if (node.type.toLowerCase().includes('scan') || node.type.toLowerCase().includes('filescan')) mappings.push(...this.mapByTableName(node));
    if (node.type.toLowerCase().includes('join')) mappings.push(...this.mapByJoinPattern(node));
    if (node.type.toLowerCase().includes('aggregate') || node.type.toLowerCase().includes('group')) mappings.push(...this.mapByAggregatePattern(node));
    if (node.type.toLowerCase().includes('exchange') || node.type.toLowerCase().includes('shuffle')) mappings.push(...this.mapByShufflePattern(node));
    return mappings;
  }
  ...
  private mapByTableName(node: DagNode): EnhancedCodeSnippet[] {
    ...
    for (const filePath of relatedFiles) {
      ...
      if (!context) continue;
      const refs = context.tableReferences.filter(ref => ref.name.toLowerCase().includes(tableName.toLowerCase()) || tableName.toLowerCase().includes(ref.name.toLowerCase()));
  ...
  private mapByShufflePattern(node: DagNode): EnhancedCodeSnippet[] {
    const mappings: EnhancedCodeSnippet[] = [];
    for (const [filePath, context] of this.contexts) {
      const shuffleCausers = context.sparkOperations.filter(op => ['join', 'aggregate', 'transform'].includes(op.type) && (op.code.includes('groupBy') || op.code.includes('repartition') || op.code.includes('join')));
  ...
  private createSnippet(context: CodeContext, line: number, node: DagNode, matchType: 'exact' | 'partial' | 'inferred', confidence: number): EnhancedCodeSnippet | null {
    ...
    return {
      filePath: context.file.path, lineNumber: line, code, relevanceExplanation: this.generateExplanation(node, matchType, context, line),
      confidence, matchType, callStack: this.traceCallStack(context, line), dependencies: this.extractDependencies(context, line)
    };
  }
  ...
  private extractDependencies(context: CodeContext, line: number): DependencyInfo[] {
    ...
    context.imports.forEach(imp => {
      if (Math.abs(imp.line - line) < 20) {
        deps.push({ type: this.isExternalLibrary(imp.module) ? 'external' : 'internal', source: imp.module, importedAs: imp.alias, usedFunctions: imp.items });
      }
    });
    return deps;
  }
  ...
  private traceCallStack(context: CodeContext, line: number): string[] {
    const stack: string[] = [];
    const containingFunc = context.functions.find(f => line >= f.startLine && line <= f.endLine);
    if (containingFunc) {
      stack.push(containingFunc.name);
      for (const func of context.functions) {
        if (func.calls.includes(containingFunc.name)) stack.unshift(func.name);
      }
    }
    return stack;
  }
  ...
  private calculateJoinMatchConfidence(node: DagNode, op: SparkOperation, context: CodeContext): number {
    ...
    if (nodeType.includes('broadcast') && op.code.toLowerCase().includes('broadcast')) confidence += 30;
    if (nodeType.includes('nestedloop') && !op.code.includes('==')) confidence += 20;
  ...
  private generateExplanation(node: DagNode, matchType: string, context: CodeContext, line: number): string {
    ...
    switch (matchType) {
      case 'exact': return `Exact match: This code directly corresponds to the ${node.type} operation ${funcName}`;
      case 'partial': return `Likely match: This code appears to trigger the ${node.type} operation ${funcName}`;
      case 'inferred': return `Inferred: This code may be related to the ${node.type} operation ${funcName}`;
      default: return `Mapped to ${node.name} ${funcName}`;
    }
  }
  ...
  private parseImport(line: string, lineNum: number): ImportStatement {
    if (line.startsWith('from ')) {
      ...
      if (match) return { module: match[1], items: [], alias: match[2], line: lineNum };
    }
  ...
  private parseFunctionDef(line: string, lineNum: number): FunctionDefinition {
    ...
    return { name, startLine: lineNum, endLine: lineNum, parameters: params, calls: [], usesDataframes: [] };
  }

  private detectSparkOperation(line: string, lineNum: number): SparkOperation | null {
    const lower = line.toLowerCase();
    if (lower.includes('.read.')) return { type: 'read', line: lineNum, code: line, dataframes: [] };
    if (lower.includes('.write.')) return { type: 'write', line: lineNum, code: line, dataframes: [] };
    if (lower.includes('.join(')) return { type: 'join', line: lineNum, code: line, dataframes: this.extractDataframesFromJoin(line) };
    if (lower.includes('.filter(') || lower.includes('.where(')) return { type: 'filter', line: lineNum, code: line, dataframes: [] };
    if (lower.includes('.groupby(') || lower.includes('.agg(')) return { type: 'aggregate', line: lineNum, code: line, dataframes: [] };
    if (lower.includes('.select(') || lower.includes('.withcolumn(')) return { type: 'transform', line: lineNum, code: line, dataframes: [] };
    return null;
  }

  private detectTableReference(line: string, lineNum: number): TableReference | null {
    const patterns = [/\.table\(['"](\w+)['"]\)/, /\.parquet\(['"].*\/(\w+)['"]\)/, /\.csv\(['"].*\/(\w+)['"]\)/, /from\s+(\w+)/i];
    for (const pattern of patterns) {
      const match = line.match(pattern);
      if (match) return { name: match[1], operation: 'read', line: lineNum, context: line };
    }
    return null;
  }
  ...
  private extractSQLTables(line: string): string[] {
    ...
    for (const match of joinMatches) tables.push(match[1]);
    return tables;
  }
  ...
  private createEmptyContext(file: RepoFile): CodeContext {
    return { file, functions: [], imports: [], tableReferences: [], sparkOperations: [] };
  }
}
